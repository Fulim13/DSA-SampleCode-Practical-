...
public class PostOfficeSim extends javax.swing.JFrame {
    ...
    private class CounterListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            if(!q.isEmpty()){
                String counterStr = e.getActionCommand();
                int counterNo = Integer.parseInt(counterStr.substring(counterNoIndex));
                Customer customer = q.poll();
                customer.setServeTime(new GregorianCalendar());
                customer.setCounter(counterNo);
                serviceList.add(customer);
                updateDisplay(customer);
                announceNumber(customer);
            } else{
                JOptionPane.showMessageDialog(null, "Not service Customer available", "Do not have customer", JOptionPane.INFORMATION_MESSAGE);
            }
    ...
    private void jbtReportActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtReportActionPerformed
        ...
        for (int i = 0; i < serviceList.size(); i++) {
            Customer c = serviceList.get(i);
            totalTime += c.getWaitingTime();
            str += String.format("%-7s %82s %15d\n",(i+1)+".", c,c.getWaitingTime());
            
        }
        str += "\n Total customers served: " + serviceList.size();
        str += String.format("\n Average waiting time: %.2f s\n",(double)totalTime/serviceList.size());
    ...
    private void jbtTakeNumberActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtTakeNumberActionPerformed
        ...
        q.offer(customer);
        jtaSlip.setText("Your number: " + customer.getSeqNo());
